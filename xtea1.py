...
class XTEA():
    ...
    def pack_key(self,key):
        ...
        for i in range(int(len(key)/4)):
            for j in range(4):
                sum_k=sum_k+ord(key[i*4+j])
            key_list.append(sum_k)
            sum_k=0
        k=struct.pack("4I", key_list[0],key_list[1],key_list[2],key_list[3])    
        return k        
        
    def xtea_encrypt(self, key,block,n=32):
        #unpack block encoding into two vectors
        v0,v1 = struct.unpack("2I",block)
        #unpack key encoding into a tuple
